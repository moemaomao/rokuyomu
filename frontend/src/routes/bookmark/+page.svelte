...
<svelte:head>
	<title>Bookmark | Mikoroku</title>
</svelte:head>

<div class="mx-auto max-w-6xl p-4 md:p-6">
	<div class="mb-6 flex items-center justify-between border-b border-zinc-800 pb-4">
		<h1 class="text-xl font-bold md:text-2xl">Daftar Bookmark</h1>
		<span class="text-sm text-zinc-400">{bookmarks.length} Manga tersimpan</span>
	</div>
	...
	{#if bookmarks.length === 0}
		<div class="flex flex-col items-center justify-center py-16 text-center text-zinc-500">
			...
			<p class="text-base font-medium">Belum ada manga yang di-bookmark.</p>
			<p class="mt-1 text-xs opacity-75">
				Jelajahi manga dan klik tombol Bookmark untuk menyimpannya di sini.
			</p>
		</div>
	{:else}
