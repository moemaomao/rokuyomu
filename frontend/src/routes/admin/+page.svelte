...
<div class="mx-auto max-w-4xl p-4 md:p-6">
    ...
    {:else if !user || !admin}
        ...
        >
            ...
            <p class="mt-2 text-sm text-zinc-500">
                Log in with an admin account to access Source Management.
            </p>
            <button
                type="button"
    ...
    {:else}
        ...
        >
            <div>
                ...
                <p class="mt-1 text-xs text-zinc-500">
                    Hide source, notes, health check, and links to user reports.
                </p>
            </div>
            <button
                type="button"
        ...
        <div class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            ...
            >
                <div class="text-xs text-amber-600 dark:text-amber-400">Reports open</div>
        ...
        {#if Object.keys(grouped).length === 0}
            <div class="py-12 text-center text-sm text-zinc-500">No matching sources found.</div>
        {:else}
            <div class="space-y-5">
                                                ...
                                                {#if editingNoteId === s.id}
                                                    <div class="mt-2 space-y-2">
                                                        <textarea
                                                            ...
                                                            rows="2"
                                                            maxlength="500"
                                                            placeholder="Admin notes (domain changed, requires cookie, etc.)"
        ...
        <p class="mt-4 space-y-1 text-xs text-zinc-500">
            <span class="block">
                <strong class="text-amber-600 dark:text-amber-400">ERROR</strong> = click to open
                <code class="rounded bg-zinc-100 px-1 dark:bg-zinc-800">/report?source=…</code>
            </span>
            <span class="block">
                <strong class="text-sky-600 dark:text-sky-400">Note</strong> stored in KV —
                examples: “domain changed”, “requires cookie”.
            </span>
            <span class="block">
                <strong class="text-emerald-600 dark:text-emerald-400">Ping</strong> = scraper health check
                (8s timeout).
            </span>
        </p>
    {/if}
