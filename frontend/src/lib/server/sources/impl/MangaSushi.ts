...
export class MangaSushiSource extends BaseSource {
	...
	protected async fetchHtml(path: string): Promise<string> {
		...
		if (!res.ok) throw new Error(`MangaSushi HTTP ${res.status} → ${url}`);
		...
		try {
			...
			for (let sp = firstSite; sp <= firstSite + 2; sp++) {
				...
				try {
					...
					console.error('[mangasushi] site page', sp, e);
					break;
				}
			...
			console.log(`[mangasushi] latest page=${p} → ${out.length}`);
			return out;
		} catch (e) {
			console.error('[mangasushi] getLatestManga', e);
			return [];
		}
	}
	...
	async searchManga(
		...
		try {
			...
			console.log(`[mangasushi] search "${q}" → ${list.length}`);
			return list;
		} catch (e) {
			console.error('[mangasushi] searchManga', e);
			return [];
		}
	}
	...
	async getMangaDetails(
		...
		if (!slug) throw new Error(`MangaSushi: invalid mangaId ${mangaId}`);
		...
		const parseChapterHtml = (chapHtml: string) => {
			...
			$$('li.wp-manga-chapter, .listing-chapters_wrap li, ul.main.version-chap li').each(
				(_, li) => {
					...
					if (chapterSlug === 'feed' || seen.has(chapterSlug)) return;
		...
		try {
			const ajaxRes = await fetch(
				`${this.baseUrl}/manga/${slug}/ajax/chapters/`,
				{
					method: 'POST',
					headers: {
						...this.h(`${this.baseUrl}/manga/${slug}/`),
			...
			if (ajaxRes.ok) {
				parseChapterHtml(await ajaxRes.text());
			}
		} catch (e) {
			console.error('[mangasushi] ajax chapters', e);
		}
	...
	async getChapterPages(chapterId: string): Promise<string[]> {
		try {
			...
			console.log(
				`[mangasushi] getChapterPages ${slug}/${chapterPath} → ${pages.length}`
			);
			return pages;
		} catch (e) {
			console.error('[mangasushi] getChapterPages', e);
			return [];
		}
	}
}
