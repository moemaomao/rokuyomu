 ...
 *
 * List/Latest : GET /api/comic/home-sections?sections=latest_projects&projectLimit=
 * Search      : GET /api/search?q=&page=&limit=
 * Detail      : GET /api/series/comic/{slug}
 *               → units[] (semua chapter sekaligus)
...
export class SoulScansSource extends BaseSource {
	...
	private readonly apiBase = 'https://img.soulscans.org';
	...
	private async apiGet<T = any>(
		...
		for (let attempt = 1; attempt <= maxAttempts; attempt++) {
			try {
				...
				if (
					...
					console.error(
						`[soulscans] HTML response attempt=${attempt}`,
						url.pathname,
						trimmed.slice(0, 120)
					);
					lastErr = new Error('SoulScans blocked: response is HTML, not JSON');
				...
				if (!res.ok) {
					console.error(
						`[soulscans] HTTP ${res.status} ${url.pathname}`,
						trimmed.slice(0, 300)
					);
					if (res.status === 429 || res.status === 503) {
						lastErr = new Error(`SoulScans HTTP ${res.status}`);
						await new Promise((r) => setTimeout(r, 600 * attempt));
						continue;
					}
					throw new Error(`SoulScans HTTP ${res.status}`);
				}

				try {
					return JSON.parse(trimmed) as T;
				} catch {
					console.error('[soulscans] invalid JSON', trimmed.slice(0, 200));
					throw new Error('SoulScans invalid JSON');
				}
			} catch (e) {
				lastErr = e;
		...
		throw lastErr instanceof Error
			? lastErr
			: new Error('SoulScans request failed');
	}
	...
	private mapListFromProject(item: any): Manga | null {
		...
		return {
			...
			type: this.mapType(item?.series_comic_type || item?.comic_subtype),
			status: this.mapStatus(item?.series_status || item?.status),
			latestChapter,
	...
	async getLatestManga(
		...
		try {
			const data = await this.apiGet<{
				...
				sections: 'latest_projects',
				projectLimit: fetchLimit
			});

			const rows = Array.isArray(data?.latest_projects)
			...
			for (const it of rows) {
				const m = this.mapListFromProject(it);
			...
			console.log(
				`[soulscans] latest (projects) page=${p} → ${pageList.length} (pool=${list.length})`
			);
			return pageList;
		} catch (e) {
			console.error('[soulscans] getLatestManga', e);
			return [];
		}
	}
	...
	async searchManga(
		...
		try {
			...
			console.log(`[soulscans] search "${q}" page=${page} → ${list.length}`);
			return list;
		} catch (e) {
			console.error('[soulscans] searchManga', e);
			return [];
		}
	}
	...
	async getMangaDetails(
		...
		if (!slug) throw new Error(`Invalid soulscans id: ${mangaId}`);
		...
		console.log(`[soulscans] details ${slug} → ch=${chapters.length}`);
	...
	async getChapterPages(chapterId: string): Promise<string[]> {
		...
		if (!seriesSlug || !chapterSlug) {
			console.error('[soulscans] getChapterPages bad id:', chapterId);
			return [];
		}
		...
		try {
			...
			console.log(
				`[soulscans] ${urls.length} pages → ${seriesSlug}/${chapterSlug}`
			);
			return urls;
		} catch (e) {
			console.error(
				'[soulscans] getChapterPages failed',
				seriesSlug,
				chapterSlug,
				e
			);
