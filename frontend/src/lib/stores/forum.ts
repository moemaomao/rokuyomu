...
interface RokuyomuDB extends DBSchema {
    chatCache: {
        key: string;
        value: ChatMessage;
    };
...
async function getLocalDB() {
    ...
    return openDB<RokuyomuDB>('rokuyomu-forum-db', 1, {
        upgrade(db) {
            if (!db.objectStoreNames.contains('chatCache')) {
                db.createObjectStore('chatCache', { keyPath: 'id' });
            }
        }
    });
}

async function getCachedChatMessages(): Promise<ChatMessage[]> {
    try {
        const idb = await getLocalDB();
        if (!idb) return [];
        const all = await idb.getAll('chatCache');
...
async function saveChatMessagesToCache(msgs: ChatMessage[]) {
    try {
        const idb = await getLocalDB();
        if (!idb) return;
        const tx = idb.transaction('chatCache', 'readwrite');
        await Promise.all([
            ...msgs.map((m) => tx.store.put(m)),
            tx.done
        ]);
    } catch (e) {
        console.error('Failed to cache chat messages to IndexedDB', e);
    }
}
// ────────────────────────────────────────────────────────────────────────────
...
// ── Community Chat (IndexedDB Enhanced) ─────────────────────────────────────

export async function getChatMessages(max = 80): Promise<ChatMessage[]> {
    if (!browser) return [];
    const cached = await getCachedChatMessages();
    if (cached.length > 0) return cached.slice(-max);
    ...
    await saveChatMessagesToCache(list);
    return list;
}
...
export function subscribeChatMessages(
    ...
    getCachedChatMessages().then((cached) => {
        if (cached.length > 0) {
            cb(cached.slice(-max));
        }
    });
