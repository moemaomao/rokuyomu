 ...
 *
 * Platform: Iken (API)
 * API base : https://api.vortexscans.org
 *
 ...
 *
 * ID format (TANPA # agar aman di URL):
 *   manga   : /series/{slug}
...
export class VortexScansSource extends BaseSource {
	...
	private readonly apiBase = 'https://api.vortexscans.org';
	...
	private async apiGet<T = any>(
		...
		if (!res.ok) {
			...
			throw new Error(
				`VortexScans HTTP ${res.status} ${url.pathname}: ${text.slice(0, 120)}`
			);
		}
	...
	async getLatestManga(
		...
		try {
			...
			console.log(`[vortexscans] latest page=${p} → ${list.length}`);
			return list;
		} catch (e) {
			console.error('[vortexscans] getLatestManga', e);
			return [];
		}
	}
	...
	async searchManga(
		...
		try {
			...
			console.log(`[vortexscans] search "${q}" page=${page} → ${list.length}`);
			return list;
		} catch (e) {
			console.error('[vortexscans] searchManga', e);
			return [];
		}
	}
	...
	async getMangaDetails(
		...
		if (!slug) throw new Error(`Invalid vortexscans id: ${mangaId}`);
		...
		const rating =
			post.averageRating != null && !Number.isNaN(Number(post.averageRating))
				...
				: null;
		const totalRatings =
			post.totalRatings != null ? Number(post.totalRatings) : null;
		...
		const metaLines = [
			...
			rating &&
				Number(rating) > 0 &&
				`Rating: ${rating}${totalRatings ? ` (${totalRatings} votes)` : ''}`,
		...
		try {
			...
			console.error('[vortexscans] chapters api', e);
		}
		...
		console.log(`[vortexscans] details ${slug} → ch=${chapters.length}`);
	...
	async getChapterPages(chapterId: string): Promise<string[]> {
		...
		if (!id) {
			console.error('[vortexscans] getChapterPages bad id:', chapterId);
			return [];
		}
		...
		try {
			...
			if (ch.isLocked || ch.isLockedByCoins || ch.isShortLinkLocked) {
				console.warn('[vortexscans] chapter locked', id);
				return [];
			}
			...
			console.log(
				`[vortexscans] getChapterPages ${id} → ${pages.length} pages`
			);
			return pages;
		} catch (e) {
			console.error('[vortexscans] getChapterPages', e);
			return [];
		}
	}
}
