 ...
 * Latest  : /manga/?order=update&page={n}
 * Search  : /?s={query}
 * Detail  : /manga/{slug}/
...
export class NgomikSource extends BaseSource {
	...
	private parseCards($: cheerio.CheerioAPI): Manga[] {
		...
		$('.listupd .bsx, .bsx, .bs .bsx').each((_, el) => {
			...
			if (/pagespeed|1\.JiBnMqyl6S|data:image/i.test(cover)) {
				cover =
					$img.attr('data-pagespeed-lazy-src') ||
					$img.attr('data-src') ||
					$img.attr('data-lazy-src') ||
			...
			const typeClass =
				$card.find('span.type').attr('class') ||
				$card.find('span[class*="type"]').attr('class') ||
				'';
			const typeText = ($card.find('span.type').text() || typeClass).toLowerCase();
			...
			let status = 'Ongoing';
			const statusText = $card.find('.status, .sts').text().toLowerCase();
			if (statusText) status = this.mapStatus(statusText);
	...
	async getLatestManga(
		...
		try {
			...
			const path =
				p <= 1
					? '/manga/?order=update'
					: `/manga/?order=update&page=${p}`;
			const html = await this.fetchHtml(path);
			const list = this.parseCards(cheerio.load(html));
			console.log(`[ngomik] latest page=${p} → ${list.length}`);
			return list.slice(0, this.PER_PAGE);
		} catch (e) {
			console.error('[ngomik] getLatestManga', e);
			return [];
		}
	}
	...
	async searchManga(
		...
		try {
			...
			const list = this.parseCards(cheerio.load(html));
			console.log(`[ngomik] search "${q}" page=${page} → ${list.length}`);
			return list;
		} catch (e) {
			console.error('[ngomik] searchManga', e);
			return [];
		}
	}
	...
	async getMangaDetails(
		...
		$('.imptdt, .fmed, .wd-full').each((_, el) => {
			...
			const label = $el.find('b, strong, h1, h2, h3').first().text().trim().toLowerCase();
			const val = $el.find('i, a, span').not('b, strong').first().text().trim();
			if (label && val) infoMap[label] = val;
		});

		const status = this.mapStatus(
			infoMap['status'] ||
				$('.imptdt:contains("Status") i, .tsinfo .imptdt i').first().text()
		);
		...
		const authorRaw =
			infoMap['author'] ||
			$('.imptdt:contains("Author") i, .fmed:contains("Author") span').text();
		if (authorRaw) {
		...
		$('.mgen a, .seriestugenre a, a[href*="/genres/"], a[href*="genre="]').each(
			(_, el) => {
				...
				if (g && !/^(manhwa|manhua|manga)$/i.test(g) && !genres.includes(g)) {
					genres.push(g);
				}
			}
		);
		...
		const alt =
			$('.alternative, .seriestualt, span.alternative').text().replace(/\s+/g, ' ').trim() ||
			'';

		const rating =
			$('.rating .numscore, .numscore, .rtb span').first().text().trim() || '';
		...
		console.log(
			`[ngomik] details ${path} → ch=${chapters.length} status=${status} type=${type}`
		);
	...
	async getChapterPages(chapterId: string): Promise<string[]> {
			...
			try {
				...
				const push = (src: string) => {
					...
					if (
						...
						!/^https?:\/\//i.test(src) ||
						src.startsWith('data:') ||
						/logo|icon|avatar|spinner|ads|banner|placeholder|gravatar|emoji|pagespeed|1\.JiBnMqyl6S/i.test(
							src
						) ||
						/\.gif(\?|$)/i.test(src)
					) {
				...
				if (images.length === 0) {
					const re =
						/https?:\/\/(?:srvr\d*\.|cover\.)?bid-cdn\.cloud\/[^"'\\\s<>]+/gi;
				...
				if (images.length === 0) {
					console.warn(
						`[ngomik] 0 pages attempt=${attempt}`,
						path,
						'htmlLen=',
						html.length
					);
					lastErr = new Error('Ngomik chapter has 0 images');
					await new Promise((r) => setTimeout(r, 400 * attempt));
					continue;
				}

				console.log(`[ngomik] ${images.length} pages → ${path}`);
				return images;
			} catch (e) {
				lastErr = e;
				console.error(`[ngomik] getChapterPages attempt=${attempt}`, path, e);
		...
		console.error('[ngomik] getChapterPages failed', path, lastErr);
		return [];
	}
}
